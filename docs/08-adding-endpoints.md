...
## 13. Генерация новых модулей (кастомный скрипт)

Вместо Plop (который вызывал проблемы с ESM) мы используем собственный генератор на Node.js. Шаблоны модулей хранятся в папке `plop-templates/module` (файлы с расширением `.hbs`), а скрипт `scripts/generate-module.js` копирует их, заменяя плейсхолдеры:

- `{{name}}` — имя модуля в нижнем регистре (например, `books`)
- `{{pascalCase name}}` — имя в PascalCase (например, `Books`)
- `{{camelCase name}}` — имя в camelCase (например, `books`)
...
Скрипт запросит имя модуля и создаст папку `src/modules/<name>` со всеми необходимыми файлами:

- `<name>.routes.ts`
- `<name>.schemas.ts`
- `<name>.repo.ts`
- `<name>.service.ts` (опционально)
- `<name>.routes.test.ts`

### Как это работает

1. Скрипт читает все `.hbs` файлы из `plop-templates/module`.
2. Для каждого файла заменяет плейсхолдеры на основе введённого имени.
3. Создаёт файл в целевой папке без расширения `.hbs`.

### Кастомизация шаблонов

Вы можете свободно редактировать шаблоны в `plop-templates/module`. Если нужны дополнительные плейсхолдеры, их можно добавить в скрипт `generate-module.js`.

### Преимущества

- Не зависит от внешних пакетов (кроме Node.js).
